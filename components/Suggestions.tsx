  ...
  return (
    <ResizablePanel>
      <AnimatePresence mode="wait">
        {content && (
          <div className="flex flex-col items-center justify-center max-w-xl p-2 my-4 space-y-4">
            {content
              .split(/\n(?=[0-9]\.)/)
              ...
              .map((item: string, i: number) => {
                return (
                  <motion.button
                    className={`p-2 transition border shadow-md rounded-xl bg-white hover:bg-gray-100`}
                    onClick={() => {
                      onSelect(`${item}`);
                    }}
                    key={`${item + i}`}
