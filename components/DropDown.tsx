...
export default function DropDown<T>({
  ...
  return (
    <Listbox multiple onChange={setValue}>
      <div className="relative block w-full text-left">
        <Listbox.Button className="inline-flex items-center justify-between w-full px-4 py-2 text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-black">
          <>
            ...
            <ChevronUpIcon
              className="hidden w-5 h-8 ml-2 -mr-1 ui-open:block"
              aria-hidden="true"
            />
            <ChevronDownIcon
              className="w-5 h-8 ml-2 -mr-1 ui-open:hidden"
              aria-hidden="true"
            />
          </>
        </Listbox.Button>
      ...
      >
        <Listbox.Options
          className="absolute left-0 z-10 mt-2 origin-top-right bg-white rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"
          key={`${value}`}
        >
